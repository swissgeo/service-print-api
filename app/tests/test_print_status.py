...
class TestPrintStatus:
    async def test_job_found_returns_200(self, client):
        ...
        with (
            ...
            response = await client.get(f"{ROOT_PATH}/jobs/{_JOB_ID}")

        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "finished"
        assert data["reportUrl"] == f"{API_BASE_URL}{ROOT_PATH}/jobs/{_JOB_ID}"
        # In prod the pdfUrl is the api-origin path served from the bucket by the proxy.
        assert data["pdfUrl"] == f"{API_BASE_URL}{ROOT_PATH}/pdf/{_JOB_ID}.pdf"
    ...
    async def test_job_not_found_returns_404(self, client):
        with patch("app.api.jobs.get_print_job", return_value=None):
            response = await client.get(f"{ROOT_PATH}/jobs/nonexistent")
...
class TestPrintList:
    async def test_returns_501(self, client):
        response = await client.get(f"{ROOT_PATH}/jobs")
